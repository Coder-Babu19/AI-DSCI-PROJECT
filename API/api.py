...
def sent_recommendation(input_sent):
  ...
  dfs['index']=df['index']
  dfs['urdu']=df['urdu']
  df.drop('urdu',axis=1,inplace=True)
  ...
  indices = pd.Series(df.text_tokens)
  ...
  def recommendations(usr_input):
    ...
    return recommended_movies,urdu,data

  eng, urdu, data = recommendations(input_sent)

  result = []

  for i in range(1,10):
    result.append(eng[i])
    # result.append(urdu[i])
...
def word_recommendation(input_word):
    ...
    df['Topic']=df['Topic']+'_top'
    corpus=[]
    for text in df.text_tokens:
      text=''.join(text)
      corpus.append(text)

    vectorizerX=TfidfVectorizer()
    vectorizerX.fit(corpus)
    doc_vector=vectorizerX.transform(corpus)
    vec = pd.DataFrame(doc_vector.toarray(), columns=vectorizerX.get_feature_names_out())

    def recommendation(input):
      query_vector=vectorizerX.transform([input])
      cosineSimilarities = cosine_similarity(doc_vector,query_vector).flatten()
      cs=pd.DataFrame(cosineSimilarities)
      cs=cs.reset_index()
      cs=cs.sort_values(0,ascending=False)
      recommended_hadees=[]
      urdu=[]
      top10=list(cs.iloc[1:11].index)
      for i in top10:
        recommended_hadees.append(list(df.Hadith)[i])
        urdu.append(list(dfs.urdu)[i])
        data=DataFrame(recommended_hadees)   
      return recommended_hadees,urdu

    eng, urdu = recommendation(input_word)
...
def create():
    result = sent_recommendation(usr_input)
...
def func():
    result = word_recommendation(usr_input)
    hadith = {'Hadiths' : result}
    print(hadith)
    return hadith
